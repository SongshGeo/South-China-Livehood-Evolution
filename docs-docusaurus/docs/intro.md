...
6. 扩散过程的人口基本守恒（母体残余低于 `min_size` 时会随母体消失）；模型不含合并机制。

> **本仓库的主产出是一篇论文**：五张图、一个表格工作簿，以及正文引用的每一个数字，
> 都由 `reports/` 下的两个 notebook 和 `paper/` 下的两个脚本产生，见[从模型到手稿]。
> 模型本身在 v2.0 经过大幅重构（删除竞争机制、增加转化控制开关、优化初始化），详见
> [变更日志](tech/changelog_v2.md)。
