...
- ~~狩猎采集者遇到其他 Hunter 时仍会合并~~（当时的说法有误：每格最多一个主体，两个 Hunter 不会同处一格，合并从未发生；`merge()` 已删除）
- 移动逻辑更简单清晰
...
#### 3.4 合并机制改进（该机制已于后续版本删除）

> **后记**：`merge()` 从未被任何代码调用，且每格唯一主体规则使合并不可能发生。
> 该方法已删除，本节仅作历史记录。
...
#### 例外情况
- ~~狩猎采集者之间仍可合并~~（当时的说法有误：每格唯一主体规则使其不可能发生；`merge()` 已删除）
- 主体检查自己当前位置时不受此限制
