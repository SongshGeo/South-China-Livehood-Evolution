...
class MultipleAnalysis:
    ...
    def detect_farmers_num_change_years(self, col: str = "farmers_num") -> pd.DataFrame:
        """Detect the change time of farmers' number in each year."""
        overrides = self.overrides.columns.to_list()
        data = self.get_dataset()
        grouped = data.groupby([*overrides, "time"])[col].mean().reset_index(level=-1)

        change_time = []
        for x, y in self.overrides.itertuples(index=False):
            tmp_d = grouped.loc[(x, y)]
            ser = pd.Series(tmp_d[col].values, index=tmp_d["time"].values)
            change_time.append(detect_breakpoints(series=ser))

        result = self.overrides.copy()
...
def main(path: str) -> None:
    ...
    detected_change_years = analysis.detect_farmers_num_change_years()
    detected_change_years.to_csv(analysis.home / "detected_change_years.csv")
