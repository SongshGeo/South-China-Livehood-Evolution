...
#
# rerun_v2 已删（2026-09-01）:
#   投稿前清理时，本机 out/ 只保留 rerun_v3。天花板从 239225 降到 191380 带来的逐条
#   差值已经量化并写进 paper/model-inventory.md 与 paper/results.json，不再需要把两批
#   数据摆在一起。若返修时审稿人要求换算前后的对照，用 `make fetch-geany-data` 从集群
#   取回 rerun_v2（那边仍是完整的 out/），或按本脚本的历史版本重投一次。
#   为参考：天花板下调后，初始布局的狩猎采集者（seed=42 实测第 0 步 183763 人）从占满
#   77% 变成占满 96%，一开局就几乎顶到上限，之后会长期被削。
#
# 设计:
